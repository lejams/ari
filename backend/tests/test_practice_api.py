...
def test_onboard_answer_resume_feedback_history_progression(
    ...
    with TestClient(create_app(demo_container)) as client:
        ...
        assert catalog["demo_enabled"] is True
        ...
        first = {
            ...
            else "Alex Beispiel ist 40 Jahre alt und kommt für eine Kommunikationsübung.",
        }
        ...
        second = {
            ...
            "text": "Beidseitig."
            if phase == "fachbegriffe"
            else "Zu Medikamenten liegen keine Angaben vor.",
        }
        ...
        with TestClient(create_app(demo_container)) as reloaded:
        ...
        assert feedback["state"] == "provisional"
        ...
        assert history[0]["feedback_state"] == "provisional"
        progress = client.get("/api/progression").json()
        assert len(progress["groups"]) == 1
        assert progress["groups"][0]["points"][0]["run_id"] == run["id"]


def test_owner_isolation_including_every_mutation(demo_container: Container) -> None:
    ...
    with TestClient(app) as alice, TestClient(app) as bob, TestClient(app) as anon:
        ...
        run = alice.post(
            "/api/practice/runs",
            json={
                "scenario_id": "ari-demo-arzt_arzt",
                "scenario_version": "1",
                "mode": "exam",
                "request_id": new_id(),
            },
...
def test_empty_catalog_and_demo_gate(container: Container) -> None:
    ...
    with pytest.raises(InvalidStateError, match="forbidden"):
        build_container(Settings(_env_file=None, environment="production", enable_mvp_demos=True))


def test_partial_and_empty_feedback_are_not_full_results(demo_container: Container) -> None:
    with TestClient(create_app(demo_container)) as client:
        client.post("/api/learners", json={})
        for answered in (False, True):
            run = client.post(
                "/api/practice/runs",
                json={
                    "scenario_id": "ari-demo-fachbegriffe",
                    "scenario_version": "1",
                    "mode": "exam",
                    "request_id": new_id(),
                },
            ...
            if answered:
                client.post(
                    path + "/answers",
                    json={"question_id": "first", "event_id": new_id(), "text": "oral"},
                )
            ...
            assert result["dimensions"][0]["answered_weight"] == (1 if answered else 0)
            assert result["dimensions"][0]["expected_weight"] == 2
            assert result["items"][-1]["evidence"] is None


def test_published_label_is_not_authorization_and_demos_never_create_reviews(
    demo_container: Container,
) -> None:
    repo = demo_container.practice.repository
    learner = demo_container.orchestrator.create_learner(CEFRLevel.B2)
    content = synthetic_demos()[0]
    forged = PracticeRun(
        id=new_id(),
    ...
    with pytest.raises(InvalidStateError, match="non publié"):
        repo.create(forged)
    with pytest.raises(InvalidStateError, match="désactivées"):
        SqlPracticeRepository(demo_container.repository.engine).create(
            forged.model_copy(update={"content": content}),
        )
    # Demos stay external to the publication store, even after successful use.
    demo_container.practice.start(learner.id, content.scenario_id, "1", "training", new_id())
    with demo_container.repository.engine.connect() as db:
        assert db.scalar(text("SELECT count(*) FROM clinical_scenarios")) == 0
        assert db.scalar(text("SELECT count(*) FROM clinical_reviews")) == 0


def test_sqlite_concurrent_retry_and_immutable_results(demo_container: Container) -> None:
    learner = demo_container.orchestrator.create_learner(CEFRLevel.C1)
    request_id = new_id()

    def start(_: int) -> str:
        return demo_container.practice.start(
            learner.id, "ari-demo-fachbegriffe", "1", "exam", request_id
        ).id
    ...
    def answer(_: int) -> int:
        return len(
            demo_container.practice.answer(
                learner.id, run_id, "first", "Beidseitig", event_id
            ).answers
        )

    with ThreadPoolExecutor(max_workers=2) as pool:
        assert list(pool.map(answer, (1, 2))) == [1, 1]
    demo_container.practice.finish(learner.id, run_id)
    for sql in (
    ...
    fresh = SqliteSessionRepository(demo_container.settings.database_url)
    assert SqlPracticeRepository(fresh.engine).get(run_id, learner.id).status == "completed"


def test_compatibility_separates_versions_modes_and_phases(demo_container: Container) -> None:
    learner = demo_container.orchestrator.create_learner(CEFRLevel.B2)
    for phase, mode in (
        ("arzt_arzt", "exam"),
        ("fachbegriffe", "exam"),
        ...
        run = demo_container.practice.start(learner.id, f"ari-demo-{phase}", "1", mode, new_id())
        demo_container.practice.finish(learner.id, run.id)
    from ari.application.services.progression import practice_progression

    series = practice_progression(demo_container.practice.repository.list(learner.id))["groups"]
    assert len(series) == 3
...
def test_future_published_content_uses_exact_reviewed_bundle_without_code_changes(
    ...
    catalog = demo_container.practice.catalog
    ...
    learner = demo_container.orchestrator.create_learner(CEFRLevel.B2)
    run = demo_container.practice.start(learner.id, scenario.id, scenario.version, "exam", new_id())
    demo_container.practice.answer(learner.id, run.id, "question-1", "Seit 1 Tag.", new_id())
    demo_container.practice.answer(
        learner.id, run.id, "question-2", "Das weiß ich nicht.", new_id()
    )
    ...
    with pytest.raises(Exception, match="Aucun exercice approuvé"):
        demo_container.practice.start(learner.id, scenario.id, scenario.version, "exam", new_id())
    assert (
        demo_container.practice.start(
            learner.id, scenario.id, scenario.version, "exam", run.request_id
        ).id
        == run.id
    )
    finished = demo_container.practice.finish(learner.id, run.id)
...
def test_new_phase_without_specification_and_unknown_rights_remain_unpublishable(
    demo_container: Container,
) -> None:
    ...
    # Runtime demos have unknown rights and no reviews, and cannot be published either.
    demo = synthetic_demos()[0].bundle
    store.import_bundle(demo)
    with pytest.raises(InvalidStateError, match="Droits non compatibles"):
        store.publish(demo.scenarios[0].id, "1", actor="ISOLATED TEST")


def test_forged_feedback_cannot_be_sealed(demo_container: Container) -> None:
    from pydantic import ValidationError

    from ari.domain.practice import assess_practice

    learner = demo_container.orchestrator.create_learner(CEFRLevel.B2)
    run = demo_container.practice.start(learner.id, "ari-demo-fachbegriffe", "1", "exam", new_id())
    ...
    with pytest.raises(ValidationError, match="Feedback incohérent"):
        demo_container.practice.repository.finish(run.id, learner.id, forged, 0)
    assert demo_container.practice.repository.get(run.id, learner.id).status == "active"
